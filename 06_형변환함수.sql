...
SELECT TO_CHAR(sysdate,'YY-MM-DD DY PM HH:MI:SS') FROM dual;
...
SELECT '2000'+2000 FROM dual --명시적 형 변환
...
SELECT TO_DATE('2024년 12월 12일','YYYY"년" MM"월" DD"일"') FROM dual;

--xxxx년 xx월 xx일 ->문자열 형식으로 변환해 보세요.
--조회 컬럼명은 dateinfo라고 하겠습니다. dual에서 조회하세요
SELECT TO_CHAR(TO_DATE('20050102','YYYY-MM-DD'),'YYYY"년" MM"월" DD"일"')AS datinfo FROM dual;
...
--NULL 형태를 변환하는 함수 NVL2(컬럼, NULL이 아닐경우의 값,NULL일 경우의 값)
SELECT 
first_name,
salary,
NVL2(
...
SELECT 
    ...
    (CASE job_id
        ...
        WHEN 'FR_ACCOUNT' THEN salary * 1.4
        ELSE salary
    END) AS result
FROM employees;
